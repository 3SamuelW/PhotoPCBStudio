...
QColor ImageProcessor::getSolderMaskColor(const QString& colorName) {
    const QString name = colorName.trimmed().toLower();
    if (name.contains("blue") || name.contains("蓝")) return QColor(20, 72, 170, 215);
    if (name.contains("black") || name.contains("黑")) return QColor(12, 12, 14, 245);
    if (name.contains("red") || name.contains("红")) return QColor(170, 28, 36, 215);
    if (name.contains("green") || name.contains("绿")) return QColor(18, 120, 72, 215);
    if (name.contains("white") || name.contains("白")) return QColor(238, 238, 232, 225);
    return QColor(20, 72, 170, 215);
}
...
QColor ImageProcessor::getMetalRenderColor(const QString& finishType) {
    bool isHASL = finishType.contains("HASL", Qt::CaseInsensitive) || finishType.contains("喷锡");
